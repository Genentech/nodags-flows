...
class NotearsClassWrapper:

    def __init__(self, lambda1=0.1, loss_type='l2', noise_scale=0.5):
        self.lambda1 = lambda1 
        self.loss_type = loss_type
        self.noise_scale = noise_scale
    
    def train(self, datasets, intervention_sets, return_weights=False, batch_size=64):
        W_est = notears_linear(datasets[0], lambda1=self.lambda1, loss_type=self.loss_type)
        self.W_est = W_est 
        if return_weights:
            return W_est
    
    def get_auprc(self, W_gt, n_points=50):
        baseline, area = compute_auprc(W_gt, np.abs(self.W_est), n_points=n_points)
        return baseline, area
    
    def computeLDG(self):
        if self.W_est.shape[0] > 20:
            print("WARNING: This method could be slow for graphs with more than 20 nodes")

        I = np.eye(self.W_est.shape[0])
        det = np.linalg.det(I - self.W_est.T)
    ...
    def computeNLL(self, x, intervention_set):
        ...
        e = x @ (I - self.W_est @ U)
        logpe = standard_normal_logprob(e[:, observed_set], noise_scale=self.noise_scale)
    ...
    def predictLikelihood(self, datasets, intervention_sets):
        nll_list = [self.computeNLL(dataset, intervention_set) / self.W_est.shape[0] for dataset, intervention_set in zip(datasets, intervention_sets)]
        return nll_list
